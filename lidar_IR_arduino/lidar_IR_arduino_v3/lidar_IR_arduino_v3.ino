...
//  ir2.data = x2_f;
  ...
  int index2 = 0;
  for(int i=0;i<9;i++){
    index1 += (x1<thre_ir1[i]);
    index2 += (x2<thre_ir2[i]);
  }
  lidar.data = pW_f;
  switch(index1){
    case 0:
...
//      ir1.data = 5+5*index1+5*(thre_ir1[index1-1]-x1_f)/(thre_ir1[index1-1]-thre_ir1[index1]);
      d1 = 5+5*index1+5*(thre_ir1[index1-1]-x1)/(thre_ir1[index1-1]-thre_ir1[index1]);
      break;
  }
  switch(index2){
    case 0:
...
//      ir2.data = 50;
      ...
      break;
    default:
//      ir2.data = 5+5*index2+5*(thre_ir2[index2-1]-x1_f)/(thre_ir2[index2-1]-thre_ir2[index2]);
      d2 = 5+5*index1+5*(thre_ir2[index2-1]-x2)/(thre_ir2[index2-1]-thre_ir2[index2]);
      break;
  }
  d1_f = lambda/(1+lambda)*d1+1/(1+lambda)*d1_fold; //필터된 값
  d1_fold = d1_f; // 센서 필터 이전값 업데이트
  d2_f = lambda/(1+lambda)*d2+1/(1+lambda)*d2_fold; //필터된 값
  d2_fold = d2_f; // 센서 필터 이전값 업데이트
  ir1.data = d1_f;
  ir2.data = d2_f;
  Lidar.publish(&lidar);
  IR1.publish(&ir1);
